...
called subarray_sum that finds the indices of a contiguous subarray in nums that add up
to the target sum using a hash table (dictionary).
...
to return the indices of any one such subarray. Also, the input list may contain both positive and negative integers.
"""
...
def subarray_sum(nums, target):
    ...
    for i, num in enumerate(nums):
        ...
        if complement in num_map:
            return [num_map[cumulative_sum - target] + 1, i]
